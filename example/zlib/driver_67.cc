...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
    ...
    if (ret != Z_OK && ret != Z_STREAM_END) {
        inflateEnd(&strm);
        return 0; // Early exit if inflate fails
    }
    ...
    if (ret != Z_OK) {
        inflateEnd(&strm);
        return 0; // Early exit if reset fails
    }
    ...
    if (ret != Z_OK) {
        inflateEnd(&strm);
        return 0; // Early exit if reset keep fails
    }

    // Reset the inflate state with a specified window size
    ret = inflateReset2(&strm, MAX_WBITS);
    if (ret != Z_OK) {
        inflateEnd(&strm);
        return 0; // Early exit if reset2 fails
    }

    // Clean up and deallocate resources
    ret = inflateEnd(&strm);
    if (ret != Z_OK) {
        return 0; // Early exit if cleanup fails
    }
