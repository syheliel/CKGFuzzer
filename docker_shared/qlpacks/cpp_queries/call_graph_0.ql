...
// where  getCallGraph(start, end, "list_free", end.getName()) or hasPath(start, end,"list_free")
...
from Function start, Function end, Location start_loc, Location end_loc
where   start.hasName("list_free") and ( getCallGraph(start, end, "list_free", end.getName())  or hasPath(start, end,"list_free") ) and  start_loc = start.getADeclarationEntry().getBlock().getLocation() and end_loc = end.getLocation()
